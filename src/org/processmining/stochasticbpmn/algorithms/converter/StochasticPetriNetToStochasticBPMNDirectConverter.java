...
public class StochasticPetriNetToStochasticBPMNDirectConverter {
    ...
    private StochasticBPMNDiagram bpmnDiagram;
    ...
    public StochasticPetriNetToStochasticBPMNDirectConverter(StochasticNet stochasticNet, Place initialPlace, Marking finalMarking) {
        if (stochasticNet == null) throw new IllegalArgumentException("'petriNet' is null");
    ...
    public StochasticBPMNDiagram convert() {
        if (bpmnDiagram != null) return bpmnDiagram;

        bpmnDiagram = new StochasticBPMNDiagramImpl("BPMN diagram for " + stochasticNet.getLabel());
    ...
    private void convertTransitionsToActivities() {
        for (Transition transition : stochasticNet.getTransitions()) {
    ...
    private Set<Place> collectEquivalentPlaces(Place place) {
        ...
        for (Place anyPlace : stochasticNet.getPlaces()) {
    ...
    private Set<Transition> collectOutTransitions(Place place) {
        ...
        Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> outEdges = stochasticNet
                .getOutEdges(place);
        for (PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> outEdge : outEdges) {
            if (!(outEdge instanceof ResetArc)) {
                outTransitions.add((TimedTransition) outEdge.getTarget());
            }
        }
    ...
    private void connectToOutTransitions(BPMNNode startNode, Place place, Set<Place> convertedPlaces) {
        ...
        StochasticGateway xorSplit = null;
        StochasticGatewayWeightedFlow stochasticGatewayWeightedFlow = new StochasticGatewayWeightedFlow();
        if (outTransitions.size() > 1) {
            xorSplit = this.bpmnDiagram.addStochasticGateway("Exclusive gateway", GatewayType.DATABASED,
                    stochasticGatewayWeightedFlow);
            Flow flow = this.bpmnDiagram.addFlow(startNode, xorSplit, (String)null);
            this.placeConversionMap.put(place, flow);
        }
        ...
        for (Transition outTransition : outTransitions) {
            ...
            if (xorSplit == null) {
                ...
                StochasticFlow f = this.bpmnDiagram.addStochasticFlow(xorSplit, activity, (String)null);
                f.getAttributeMap().put("Original id", uuid);
                if (outTransition instanceof TimedTransition) {
                    TimedTransition timedTransition = (TimedTransition)outTransition;
                    stochasticGatewayWeightedFlow.assignFlowWeight(BigDecimal.valueOf(timedTransition.getWeight()), uuid);
                }
    ...
    private boolean isFinalPlace(Place place) {
        for (PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> e : stochasticNet.getOutEdges(place)) {
    ...
    private Set<Transition> collectInTransitions(Place place) {
        ...
        Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> inEdges = stochasticNet
                .getInEdges(place);
    ...
    private BPMNNode connectToInTransition(Transition inTransition, BPMNNode splitNode) {
        ...
        if (stochasticNet.getOutEdges(inTransition).size() > 1) {
        ...
        } else if (stochasticNet.getOutEdges(inTransition).size() == 1) {
            if (splitNode != null) {
